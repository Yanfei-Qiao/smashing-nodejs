define('cookieLib', ['jquery'], function(require, exports, module) {
  /**
  ...
  var deteleCookie =  function (name) {
    var isArray = Array.isArray(name)
    var keys = document.cookie.match(/[^ =;]+(?==)/g)
    keys.forEach(function (k) {
      if (!name || isCurrentName(k)) {
        document.cookie = k + '=0;expires=' + new Date(0).toUTCString()+";domain=.xinhehui.com;path=/";
      }
    })
    // 是不是当前name
  ...
  exports.deteleCookie = deteleCookie;
});